...
CLASS lcl_agency_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_agency TYPE STANDARD TABLE OF /dmo/agency19 WITH KEY agency_id.
...
CLASS lcl_airport_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_airport TYPE STANDARD TABLE OF /dmo/airport19 WITH KEY airport_id.
    CLASS-METHODS: get_data
...
CLASS lcl_airport_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/airport19.                           "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data(  ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/airport19 FROM TABLE @lt_data.
...
CLASS lcl_carrier_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_carrier TYPE STANDARD TABLE OF /dmo/carrier19 WITH KEY carrier_id.
    CLASS-METHODS: get_data
      RETURNING
...
CLASS lcl_carrier_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/carrier19.                           "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/carrier19 FROM TABLE @lt_data.
...
CLASS lcl_connection_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_connection TYPE STANDARD TABLE OF /dmo/connecti_19 WITH KEY carrier_id connection_id.
...
CLASS lcl_connection_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ). ENDIF.
    DELETE FROM /dmo/connecti_19.                        "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ).  ENDIF.
    INSERT /dmo/connecti_19 FROM TABLE @lt_data.
...
CLASS /dmo/cl_flight_data_generat_19 DEFINITION LOCAL FRIENDS lcl_flight_data_generator.
CLASS lcl_flight_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_flights TYPE STANDARD TABLE OF /dmo/flight19
  ...
  PRIVATE SECTION.
    TYPES: BEGIN OF ty_plane_type,
             id            TYPE /dmo/plane_type_id19,
             seats_max     TYPE /dmo/plane_seats_max19,
           ...
           BEGIN OF ty_flight_info,
             ...
             long_distance  TYPE abap_bool,
             seats_max      TYPE /dmo/plane_seats_max19,
             seats_occupied TYPE /dmo/plane_seats_occupied19,
             price          TYPE /dmo/flight_price19,
           END OF ty_flight_info,
           BEGIN OF ty_connection_recurrency,
             id         TYPE /dmo/connection_id19,
             recurrency TYPE STANDARD TABLE OF /dmo/flight_date19 WITH EMPTY KEY,
    ...
    CLASS-METHODS: build_dependent_content,
      get_flight_info
        IMPORTING
          iv_connection_id     TYPE /dmo/connection_id19
        RETURNING
...
CLASS lcl_flight_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/flight19.                            "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ).  ENDIF.
    INSERT /dmo/flight19 FROM TABLE @gt_flights.
    ...
    LOOP AT gt_connections INTO DATA(ls_connection).
      LOOP AT gt_connection_recurrency[ id = ls_connection-connection_id ]-recurrency INTO DATA(lv_flight_date).
        ...
        APPEND VALUE /dmo/flight19(
                  ...
                  currency_code  = VALUE /dmo/flight19-currency_code(
  ...
  METHOD get_flight_info.
    ...
    rs_plane_info = VALUE ty_flight_info(
        ...
        price          = /dmo/cl_flight_data_generat_19=>calculate_flight_price(
  ...
  METHOD build_connection_recurrency.
    ...
    flight_date_max = calc_next_monday( CONV /dmo/flight_date19( lv_datum + 217 ) ).

    " flight_date_min is a Monday 5 months in the past
    flight_date_min = calc_next_monday( CONV /dmo/flight_date19( lv_datum - 148 ) ).
      ...
      APPEND VALUE ty_connection_recurrency(
          ...
          recurrency = VALUE ty_connection_recurrency-recurrency(
                                    ...
                                    UNTIL flightdate < flight_date_min + ls_connection-weekday - 1
                                       ( CONV /dmo/flight_date19( flightdate ) )
                               )
      ) TO rt_data.
    ENDLOOP.
  ENDMETHOD.
...
CLASS lcl_customer_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_last_name,
             last_name TYPE /dmo/last_name19,
           END OF ty_last_name.
    TYPES: tt_customer  TYPE STANDARD TABLE OF /dmo/customer19 WITH KEY customer_id,
  ...
  PRIVATE SECTION.
    TYPES:
      ...
      BEGIN OF ty_first_name,
        first_name TYPE /dmo/first_name19,
        gender     TYPE c LENGTH 1,
      END OF ty_first_name,
      BEGIN OF ty_name,
        first_name TYPE /dmo/first_name19,
        last_name  TYPE /dmo/last_name19,
        title      TYPE /dmo/title19,
      END OF ty_name,
      " Addresses
      BEGIN OF ty_city,
        country     TYPE land1,
        postal_code TYPE /dmo/postal_code19,
        city        TYPE /dmo/city19,
      END OF ty_city,
      tt_street_per_country TYPE STANDARD TABLE OF /dmo/street19 WITH EMPTY KEY,
      BEGIN OF ty_street,
        country TYPE land1,
      ...
      BEGIN OF ty_address,
        ...
        postal_code   TYPE /dmo/postal_code19,
        city          TYPE /dmo/city19,
        street        TYPE /dmo/street19,
        phone_number  TYPE  /dmo/phone_number19,
        email_address TYPE /dmo/email_address19,
      END OF ty_address.
...
CLASS lcl_customer_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/customer19.                          "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/customer19 FROM TABLE @lt_data.
    ...
    LOOP AT build_last_names( ) INTO DATA(ls_last_name).
      DO lo_random_counter->get_next( ) TIMES.
        ...
        APPEND VALUE ty_name(
            ...
            title      = COND /dmo/title19( WHEN ls_first_name-gender = 'M' THEN 'Mr.'
...
CLASS lcl_supplement_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_supplement      TYPE STANDARD TABLE OF /dmo/suppleme_19 WITH KEY supplement_id,
           tt_supplement_text TYPE STANDARD TABLE OF /dmo/suppl_te_19 WITH KEY supplement_id.

    " Merged types
    TYPES BEGIN OF ty_supplement_complete.
    INCLUDE TYPE /dmo/suppleme_19.
    TYPES language_code TYPE spras.
    TYPES description   TYPE /dmo/description19.
...
CLASS lcl_supplement_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/suppleme_19.                        "#EC CI_NOWHERE
    DELETE FROM /dmo/suppl_te_19.                        "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/suppleme_19 FROM TABLE @( CORRESPONDING tt_supplement( lt_data ) ).
    INSERT /dmo/suppl_te_19 FROM TABLE @( CORRESPONDING tt_supplement_text( lt_data ) ).
...
CLASS /dmo/cl_flight_data_generat_19 DEFINITION LOCAL FRIENDS lcl_travel_data_generator.
CLASS lcl_travel_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.

    TYPES: tt_travel              TYPE STANDARD TABLE OF /dmo/travel19     WITH KEY travel_id,
           tt_bookings            TYPE STANDARD TABLE OF /dmo/booking19    WITH KEY travel_id booking_id,
           tt_booking_supplements TYPE STANDARD TABLE OF /dmo/book_sup_19 WITH KEY travel_id booking_id booking_supplement_id.

    " Build nested tables
    TYPES BEGIN OF ty_booking_complete.
    INCLUDE TYPE /dmo/booking19.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS:
      ...
      build_booking
        IMPORTING
          iv_travel_id       TYPE /dmo/booking19-travel_id
        RETURNING
      ...
      find_next_fitting_flight
        IMPORTING
          ...
          is_flight_previous TYPE /dmo/flight19 OPTIONAL
        RETURNING
          VALUE(rs_flight)   TYPE /dmo/flight19,
      generate_booking_supplements
        IMPORTING
          iv_travel_id   TYPE /dmo/booking19-travel_id
          iv_booking_id  TYPE /dmo/booking19-booking_id
        RETURNING
      ...
      generate_description
        ...
        RETURNING
          VALUE(rv_description) TYPE /dmo/travel19-description,
      ...
      calculate_booking_fee
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_booking_fee) TYPE /dmo/travel19-booking_fee,
      generate_travel_customer_id
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_customer_id) TYPE /dmo/travel19-customer_id,
      set_today,
...
CLASS lcl_travel_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    ...
    LOOP AT lt_data INTO DATA(ls_travel).
      APPEND CORRESPONDING /dmo/travel19( ls_travel ) TO lt_travels.
      LOOP AT ls_travel-bookings INTO DATA(ls_booking).
        APPEND CORRESPONDING /dmo/booking19( ls_booking ) TO lt_bookings.
    ...
    INSERT /dmo/travel19 FROM TABLE @lt_travels.
    IF out IS BOUND.  out->write( '--> Insert Booking Content' ).  ENDIF.
    INSERT /dmo/booking19 FROM TABLE @lt_bookings.
    IF out IS BOUND.  out->write( '--> Insert Booking Supplement Content' ).  ENDIF.
    INSERT /dmo/book_sup_19 FROM TABLE @lt_booking_supplements.
  ...
  METHOD get_data.
    DATA: lv_travel_id TYPE /dmo/booking19-travel_id.
    ...
    WHILE lt_bookings IS NOT INITIAL.
      ...
      APPEND VALUE ty_travel_complete(
          ...
          total_price   = lv_booking_fee + REDUCE /dmo/travel19-total_price( INIT sum = 0
                                                          FOR booking IN lt_bookings
                                                          NEXT
                                                           sum = sum
                                                                 + booking-flight_price
                                                                 + REDUCE /dmo/flight_price19( INIT sum_supplement = 0
          ...
          status        = SWITCH /dmo/travel19-status( go_ran_status_description->get_next( )
                                          WHEN 1 OR 2 THEN /dmo/if_flight_legacy19=>travel_status-new
                                          WHEN 3      THEN /dmo/if_flight_legacy19=>travel_status-booked
                                          WHEN 4      THEN /dmo/if_flight_legacy19=>travel_status-planned  )
  ...
  METHOD build_booking.
    TYPES: tt_customer_id TYPE TABLE OF /dmo/customer19-customer_id WITH EMPTY KEY.
    DATA: lv_booking_id      TYPE /dmo/booking19-booking_id,
    ...
    DO lv_trip_length TIMES.
      ...
      DATA(lv_booking_date) = CONV /dmo/booking19-booking_date( <flight>-flight_date - go_ran_booking_date->get_next( ) ).

      DATA(lv_price) = /dmo/cl_flight_data_generat_19=>calculate_flight_price(
      ...
      FOR i = 1  THEN i + 1  WHILE i <= lines( lt_customer_id )
         (
            ...
            booking_supplements = generate_booking_supplements( iv_travel_id = iv_travel_id   iv_booking_id = CONV /dmo/booking19-booking_id( lv_booking_id + i ) )
          )
      ) TO rt_bookings.
  ...
  METHOD find_next_fitting_flight.
    ...
    IF is_flight_previous IS SUPPLIED.
      DATA(lv_connection_id_new) = VALUE /dmo/connecti_19-connection_id( gt_connections[
  ...
  METHOD generate_description.
    TYPES: tt_customers TYPE SORTED TABLE OF /dmo/customer19 WITH UNIQUE KEY customer_id.
    rv_description = SWITCH /dmo/travel19-description(
                               go_ran_travel_description->get_next( )
                                  WHEN 1 THEN `Business Trip for ` &&
                                                REDUCE /dmo/travel19-description(
                                                               ...
                                                               NEXT s =  s && gt_customer[ customer_id = traveler-customer_id ]-first_name
                                                                           && COND /dmo/travel19-description( WHEN i < lines( it_bookings ) THEN `, ` )
                                                                    i = i + 1 )
  ...
  METHOD generate_travel_customer_id.
    rv_customer_id = COND /dmo/travel19-customer_id(
